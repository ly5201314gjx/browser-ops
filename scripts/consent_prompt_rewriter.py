...
if __name__ == "__main__":
    if len(sys.argv) not in (3, 4):
        ...
        raise SystemExit(1)
    out = rewrite_prompt(sys.argv[1], sys.argv[2], sys.argv[3] if len(sys.argv) == 4 else None)
    print(json.dumps({"ok": True, "output": str(Path(sys.argv[1]) / 'consent_prompt_rewrite.json'), "deviceProfile": out['deviceProfile']}, ensure_ascii=False))
