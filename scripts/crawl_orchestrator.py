...
MVP+: initializes task folder/state and executes the HTTP route directly,
or generates a browser action plan for browser/hybrid routes.
"""
...
def main(profile_path: str, out_dir: str | None = None):
    ...
    if route == "http":
        proc = subprocess.run(
            [sys.executable, str(Path(__file__).with_name("http_crawler.py")), profile_path, str(task_dir), "list"],
            capture_output=True,
            text=True,
        )
    ...
    if route in {"browser", "hybrid", "human"}:
        proc = subprocess.run(
            [sys.executable, str(Path(__file__).with_name("browser_plan_builder.py")), profile_path, str(task_dir)],
            capture_output=True,
            text=True,
        )
        print(proc.stdout.strip() or proc.stderr.strip())
