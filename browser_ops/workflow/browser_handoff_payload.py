...
def main(argv: list[str]) -> int:
    if len(argv) != 2:
        ...
        return 1
    result = build_payload(argv[1])
    print(json.dumps({"ok": True, "output": str(Path(argv[1]) / 'browser_handoff_payload.json'), "browserSteps": len(result['browserSteps'])}, ensure_ascii=False))
    return 0
