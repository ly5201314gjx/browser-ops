...
<p align="center">
  <b>不是浏览器脚本集合，不是一次性自动化玩具，也不是只会点网页的 Demo 仓库。</b><br/>
  <b>这是一个专为 OpenClaw 打造的、由情报驱动的浏览器操作平台（Browser Operations Platform）。🔥</b><br/>
  <b>它会先理解网站，再决定路线、策略、交接方式、自动推进节奏与失败恢复方案。</b>
</p>
...
**Browser Ops** 是一个面向真实网站研究、深层解析、自动化决策、人机协作、失败恢复与持续推进的浏览器操作平台。  
它不是“打开网页然后点按钮”的脚本拼盘，而是一套把浏览器任务当成**系统问题**来处理的工程内核。

它解决的不是“能不能点”，而是：

> **面对不同网站、不同复杂度、不同拦截环境时，AI 应该如何先理解、再决策、再执行、再恢复。**

现在这个项目已经具备以下核心能力：

- **Site Intelligence（站点侦察）**：识别页面类型、嵌入数据、分页信号、登录/MFA/Captcha/审批墙，并推荐最优路线（HTTP / Browser / Hybrid / Human）
- **Deep Analysis（深层次解析）**：从 8 个维度对网站进行深挖，输出 `deep_analysis.json` 与 `deep_report.md`
- **Action Policy（动作策略）**：根据设备画像、站点复杂度、风险等级和深层解析结果动态调整交互节奏与保守程度
- **Runbook / Handoff / Recovery**：把浏览器任务组织成可观察、可恢复、可交接、可继续推进的任务系统
- **Autopilot（自动推进）**：不是无脑执行，而是会基于深层解析结果判断是否继续自动跑、暂停给人、还是切换策略
- **Human-in-the-Loop（人机协作）**：遇到登录、验证码、MFA、审批门等保护墙时自动进入人工接管模式，而不是硬撞

简而言之，这不是一个普通浏览器自动化仓库。  
这是一个正在成型的：

> **情报驱动浏览器操作内核（Intelligence-Driven Browser Operations Core）**

---

## 核心算法 ⚙️

Browser Ops 的主算法不是“写死脚本然后执行”，而是一个多阶段决策链：
...
- 有没有嵌入数据 / 分页信号
- 有没有登录、验证码、MFA、审批门
- 最适合走哪条路线：`http / browser / hybrid / human`

### 2. Deep Analysis（8维深层解析）
系统会从以下 8 个维度深挖站点：
1. **页面结构**
2. **数据流**
3. **网络依赖**
...
- risk tolerance
- page settle timing
- between-action timing
- login/MFA/captcha/approval 暂停策略

### 4. Runbook-Oriented Execution
把浏览器任务拆成结构化 runbook，而不是一次性脚本。  
这样就能做到：
- 可恢复
- 可暂停
- 可交接
...
### 5. Handoff + Recovery + Autopilot
系统不仅能执行，还能：
- 在保护墙前自动停下并生成人工交接包
- 在失败后根据 deep analysis 判断该不该重试
- 在高难度 / 高摩擦场景下自动选择 `pause-for-human`

所以 Browser Ops 的核心不是“自动点网页”，而是：

> **先理解网站，再决定是否自动化、如何自动化、自动到什么程度、失败后如何继续。**

---

## 快速入口 ⚡

如果你是第一次进这个仓库，别先被长文案带偏，先看这 5 件事：

1. **技能入口**：`SKILL.md`
2. **核心脚本目录**：`scripts/`
3. **新包骨架**：`browser_ops/`
4. **参考手册**：`references/`
5. **项目体检**：`python3 scripts/doctor.py`

### 推荐阅读顺序
- 想安装体验：看 `install.sh` / `demo.sh`
- 想理解架构：看 `docs/architecture.md`
- 想知道技能怎么驱动：看 `SKILL.md`
- 想改代码：先看 `browser_ops/` 和 `scripts/browser_ops_orchestrator.py`

### 当前工程状态
这个项目已经具备：
- 可运行脚本体系
- 可安装 CLI 入口
- demo / doctor / smoke test
- references 文档层

当前正在从“脚本集合”收束到“更清晰的包结构”。
因此：
- **旧脚本入口仍保留兼容**
- **新代码优先往 `browser_ops/` 包里沉**
