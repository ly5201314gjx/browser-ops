...
def build_recovery_plan(task_dir: str) -> dict:
    ...
    if open_incidents:
        ...
        if allowed and not human_needed and automation_difficulty <= 3:
            actions.append({
            ...
            actions.append({
                "kind": "human-review",
                "priority": 2,
                "reason": f"Incident at stage {latest.get('stage')} requires review. {retry_note} Deep analysis: automationDifficulty={automation_difficulty}/5, humanCheckpointNeeded={human_needed}.",
                "incidentId": latest.get("id"),
                "category": latest.get("category"),
            })
...
def main(argv: list[str]) -> int:
    ...
    cmd = argv[1]
    if cmd == "register":
        stage = argv[3] if len(argv) > 3 else "unknown"
        reason = argv[4] if len(argv) > 4 else "unknown"
        url = argv[5] if len(argv) > 5 else ""
        snapshot = argv[6] if len(argv) > 6 else ""
        screenshot = argv[7] if len(argv) > 7 else ""
        retryable = (argv[8].lower() != "false") if len(argv) > 8 else True
        print(json.dumps(register_failure(argv[2], stage, reason, url, snapshot, screenshot, retryable), ensure_ascii=False, indent=2))
        return 0
    if cmd == "plan":
        print(json.dumps(build_recovery_plan(argv[2]), ensure_ascii=False, indent=2))
        return 0
    if cmd == "resolve":
        incident_id = argv[3]
        note = argv[4] if len(argv) > 4 else "resolved"
        print(json.dumps(resolve_incident(argv[2], incident_id, note), ensure_ascii=False, indent=2))
        return 0
    if cmd == "bump-retry":
        incident_id = argv[3]
        print(json.dumps(bump_retry(argv[2], incident_id), ensure_ascii=False, indent=2))
        return 0
    if cmd == "status":
